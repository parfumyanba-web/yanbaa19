    ...
    <div className="space-y-6">
      {orders && orders.length > 0 ? (
        orders.map((order) => (
          <div key={order.id} className="glass-card hover:border-gold/30 transition-all overflow-hidden border border-white/5 bg-white/2">
            <div className="p-8 grid grid-cols-1 md:grid-cols-4 gap-8">
              ...
              <div className="space-y-2">
                 <p className="text-[10px] uppercase text-white/30 tracking-widest ">{t('status')}</p>
                 ...
                 <p className="text-[10px] text-white/30 uppercase">{t('estimated_delivery')}</p>
              </div>
        ...
        <div className="py-20 text-center glass-card">
          ...
          <p className="text-white/20 uppercase tracking-[0.2em]">{t('no_orders_yet')}</p>
          <Link href="/store" className="text-gold text-xs font-bold uppercase mt-4 hover:underline inline-block">
            {t('start_shopping')}
          </Link>
        </div>
      )}
